...
public class BookManager {
    ...
    public List<Book> findBooksByAuthor(String penulis) {
        ...
        return Buku.stream()
                .filter(book -> book.getPenulis()
                        .equalsIgnoreCase(penulis.trim()))
    ...
    public List<Book> findBooksByYear(int buku) {
        if (buku < 2000 || buku > 2100) {
            throw new IllegalArgumentException(
                    "Tahun hanya bisa diisi dari tahun 2000 sampai 2100"
            );
        }
        return Buku.stream()
    ...
    public boolean containsBook(String judul) {
        ...
        return Buku.stream()
                .anyMatch(book -> book.getJudul_Buku()
                        .equalsIgnoreCase(judul.trim()));
    }
